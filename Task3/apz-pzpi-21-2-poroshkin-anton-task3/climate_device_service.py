...
class ClimateDeviceService:
    ...
    def convert_payload_units(self, value, current_units, desired_untis, device):
        if (device.type == ClimateDeviceType.THERMOSTAT):
            return self.units_converter.convert_temperature(value, current_units, desired_untis)
        else:
            return self.units_converter.convert_humidity(value, current_units, desired_untis)
